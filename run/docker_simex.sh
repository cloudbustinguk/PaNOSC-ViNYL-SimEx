...
FAKE_HOME=$HOME/SimEx-docker
mkdir -p $FAKE_HOME
docker run -ti -e QT_X11_NO_MITSHM=1 -e JUPYTER_RUNTIME_DIR=/tmp -v $XSOCK:$XSOCK -v XAUTH:$XAUTH -e DISPLAY:${DISPLAY_TO_USE} -v $FAKE_HOME:/home/jovyan -e XAUTHORITY=$XAUTH $DEVICESTRING -p 8080:8080 $containername jupyter notebook --allow-root --no-browser --ip 0.0.0.0 --port 8080 /home/jovyan
